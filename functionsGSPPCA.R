...
Occam <- function(x,u,d,pmin=d+1,pmax=ncol(x)-1,method="BC",nbcores=1){
  ...
  if (nbcores==1) l = sapply(dims,computelogmarginal)
  if (nbcores>1) l = parallel::mcmapply(computelogmarginal,dims,mc.cores = nbcores)
...
gsppca <- function (x,d,nit=200,alpha=c(1,10),s=1,epsi=1e-5,scale=FALSE,numvar="Occam",pmin=(d+1),pmax=ncol(x)-1,method="BC",nbcores=1,svdfree=FALSE,verbose=FALSE){
  ...
  if (length(alpha)>1){
    ...
    dvs = RSpectra::svds(scale(x,scale=scale),k=d)
    outs <- lapply(alpha, function(a) VEM(x=x,d=d,nit=3,alpha=a,s=s,mu=dvs$u,M=dvs$v,epsi=epsi,scale=scale,verbose=FALSE))
  ...
  if (length(alpha)>1) out <- VEM(x=x,d=d,nit=nit,alpha=bestout$alpha,s=bestout$s,epsi=epsi,scale=scale,verbose=verbose,u=bestout$u,Sigma=bestout$Sigma,mu=bestout$mu,S=bestout$S,M=bestout$M)
  if (length(alpha)==1) out <- VEM(x=x,d=d,nit=nit,alpha=alpha,scale=scale,verbose=verbose)
  ...
  if (numvar=="Occam") { if (verbose==TRUE) {cat('\n'); cat('Model Selection...')}
    res <- Occam(x=x,u=u,d=d,pmin=pmin,pmax=pmax,method=method,nbcores=nbcores)
  ...
  if (numvar=="Occam") {
    return(list(l=l,vhat=vhat,qhat=qhat,u=u,loadings=loadings,scores=outpca$x[,1:d],center=center))}
  else return(list(vhat=vhat,qhat=qhat,loadings=loadings,scores=outpca$x[,1:d],center=center))
}
